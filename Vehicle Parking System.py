...
class Parked_vehicle():
    ...
    def __init__(self,veh_seq,four_wheeler,parked_for,valet_parking,parked_stat="Parked"):
        self.veh_seq = veh_seq
        self.four_wheeler = four_wheeler
        self.parked_for = parked_for
        self.valet_parking = valet_parking
        
        
class Parking_lot():
    
    def __init__(self,parked_veh):
        self.parked_veh = parked_veh
    
    
    def update_parked_stat(self,lot_no):
        
        for k,v in self.parked_veh.items():
            if(k == lot_no):
                v.parked_stat = "Cleared"
                print(k,v.parked_stat)
        
    def get_parking_stat(self,lot_no):
        
        for k,v in self.parked_veh.items():
            if(k == lot_no):
                fee = 0.0
                if(v.four_wheeler.lower() == "yes"):
                    fee = 50 * v.parked_for
                    if(v.valet_parking.lower() == "yes"):
                        fee = fee + 10
                elif(v.four_wheeler.lower() == "no"):
                    fee = 30 * v.parked_for
                    if(v.valet_parking.lower() == "yes"):
                        fee = fee + 10
                print(fee)
...
if __name__ == '__main__':
    ...
    for i in range(n):
        veh_seq = int(input())
        four_wheeler = input()
        parked_for = float(input())
        valet_parking = input()
        lt = int(input())
        obj = Parked_vehicle(veh_seq,four_wheeler,parked_for,valet_parking)
        d1.update({lt:obj})
    lot_no = int(input())
    o = Parking_lot(d1)
    o.update_parked_stat(lot_no)
    o.get_parking_stat(lot_no)