...
class Doctor():
    ...
    def __init__(self,idd,name,specs,fee):
        self.idd = idd
        self.name = name
        self.specs = specs
        self.fee = fee
...
class Hospital():
    ...
    def search_by_doctor_name(self,nm):
        l1 = []
        for k,v in self.db.items():
            if(nm.lower() == v.name.lower()):
                l1.append(v)
        if(len(l1) == 0):
            print("No Doctor Exists with the given Doctor Name")
        else:
            for i in l1:
                print(i.idd,i.name,i.specs,i.fee)
        
    def Calculate_consultation_fee_by_spec(self,spec):
        
        s = 0
        for k,v in self.db.items():
            if(spec.lower() == v.specs.lower()):
                s = s + v.fee
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        specs = input()
        fee = int(input())
        obj = Doctor(idd,name,specs,fee)
        db.update({idd:obj})
    o = Hospital(db)
    nm = input("Provide Name: ")
    spec = input("Provide Spec: ")
    o.search_by_doctor_name(nm)
    o.Calculate_consultation_fee_by_spec(spec)